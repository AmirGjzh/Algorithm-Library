...
• If D is even, there exists a vertex u in T such that for every vertex v in T, the distance between v and u is at most D/2.
• If D is odd, there exists an edge e in T such that for every vertex u in T, the distance between u and
one of the endpoints of e is at most (D−1)/2.

The vertex u or the edge e is called a center of the tree.
============================================================================================================*/
...
struct SegmentTree {
    ...
    explicit SegmentTree(const int n): n(n), st(n << 2, 0), lazy(n << 2, 0) {}
    void apply(const int node, const int l, const int r, const ll val) {
        st[node] += (r - l + 1) * val;
        lazy[node] += val;
    }
    void push(const int node, const int l, const int r) {
        if (lazy[node] != 0) {
            const int mid = (l + r) >> 1;
    ...
    void update(const int node, const int l, const int r, const int ql, const int qr, const ll val) {
    ...
    ll query(const int node, const int l, const int r, const int ql, const int qr) {
    ...
    void update(const int l, const int r, const ll val) {update(1, 0, n - 1, l, r, val);}
    ll query(const int l, const int r) {return query(1, 0, n - 1, l, r);}
};
...
struct HeavyLightDecomposition {
    ...
    explicit HeavyLightDecomposition(const vector<vector<int>> &G): n(static_cast<int>(G.size())), G(G) {
    ...
    void decompos(const int u, const int h) {
        head[u] = h; pos[u] = cur_pos++;
        if (heavy[u] != -1) decompos(heavy[u], h);
        for (const int v : G[u]) if (v != par[u] and v != heavy[u])
        decompos(v, v);
    }
    ...
    void update_path(int u, int v, const ll val) const {
        while (head[u] != head[v])
        ...
        const int l = min(pos[u], pos[v]), r = max(pos[u], pos[v]);
        seg->update(l, r, val);
    }
    void update_subtree(const int u, const ll val) const {
        seg->update(pos[u], pos[u] + sz[u] - 1, val);
    }
    [[nodiscard]] ll query_subtree(const int u) const {
        return seg->query(pos[u], pos[u] + sz[u] - 1);
    }
    [[nodiscard]] ll query_path(int u, int v) const {
        ll res = 0;
        while (head[u] != head[v])
            if (depth[head[u]] > depth[head[v]]) {
...
struct DsuOnTree {
    ...
    explicit DsuOnTree(const vector<vector<int>> &G): n(static_cast<int>(G.size())), G(G) {
        sz.assign(n, 1);
        ans.assign(n, -1);
    }
    ...
    void DFS_size(const int u, const int p) {
        for (const int v : G[u]) if (v != p) {
            DFS_size(v, u);
            sz[u] += sz[v];
        }
    }
    void add(const int u, const int p, const int big_child) {
        /* Add `u` to DS */
        DS[col[u]]++;
        for (const int v : G[u]) if (v != p and v != big_child) add(v, u, big_child);
    }
    void remove(const int u, const int p) {
        /* Remove `u` from DS */
        DS[col[u]]--;
        if (DS[col[u]] == 0) DS.erase(col[u]);
        for (const int v : G[u]) if (v != p) remove(v, u);
    }
    void DFS_solve(const int u, const int p, const bool keep) {
        int max_size = 0, big_child = -1;
        for (const int v : G[u]) if (v != p and sz[v] > max_size)
            max_size = sz[v], big_child = v;
        for (const int v : G[u]) if (v != p and v != big_child) DFS_solve(v, u, false);
...
struct DsuOnTree {
    ...
    explicit DsuOnTree(const vector<vector<int>> &G): n(G.size()), G(G) {
        sz.assign(n, 1);
        ans.assign(n, -1);
    }
    ...
    void DFS_solve(const int u, const int p) {
        int max_size = 0, big_child = -1;
        for (const int v : G[u]) if (v != p and sz[v] > max_size)
            max_size = sz[v], big_child = v;
        if (big_child != -1) DFS_solve(big_child, u), DS[u] = DS[big_child];
        else DS[u] = new map<int, int>();
        for (const int v : G[u]) if (v != p and v != big_child) {
            DFS_solve(v, u);
...
struct CentroidDecomposition {
    ...
    int DFS_size(const int u, const int p) {
        size[u] = 1;
        for (const int v : G[u]) if (v != p and !is_removed[v])
            size[u] += DFS_size(v, u);
        return size[u];
    }
    int centroid(const int u, const int p, const int tree_size) {
        for (const int v : G[u]) {
    ...
    void decompos(const int u, const int p) {
        const int cent = centroid(u, -1, DFS_size(u, -1));
        /* solve the things that depends on `u`  */
        is_removed[cent] = true;
        centroid_parent[cent] = p;
        for (const int v : G[cent]) if (!is_removed[v]) decompos(v, cent);
    }
    void update(const int u) const {
        /* propagate an “update” from node u up through all centroid ancestors */
        for (int v = u; v != -1; v = centroid_parent[v]) {

        }
    }
    [[nodiscard]] int answer(const int u) const {
